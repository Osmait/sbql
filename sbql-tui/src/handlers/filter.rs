    ...
    match key.code {
        KeyCode::Esc => {
            if state.filter.show_suggestions {
                ...
                Action::Filter(FilterAction::Close)
            }
        }
        KeyCode::Up => {
            if state.filter.show_suggestions && !state.filter.suggestions.is_empty() {
                Action::Filter(FilterAction::SuggestionUp)
            } else {
                Action::Filter(FilterAction::Input(Input::from(key)))
            }
        }
        KeyCode::Down => {
            if state.filter.show_suggestions && !state.filter.suggestions.is_empty() {
        ...
        KeyCode::Tab => Action::Filter(FilterAction::ApplySuggestion),
        KeyCode::Enter => {
            ...
            if state.filter.show_suggestions && !state.filter.suggestions.is_empty() {
                ...
                if replacement != current {
                    return Action::Filter(FilterAction::ApplySuggestion);
                }
            }
            Action::Filter(FilterAction::Apply)
        }
        _ => Action::Filter(FilterAction::Input(Input::from(key))),
    }
}
...
mod tests {
    ...
    fn up_with_suggestions_moves_up() {
        ...
        assert!(matches!(act, Action::Filter(FilterAction::SuggestionUp)));
    }
    ...
    fn down_with_suggestions_moves_down() {
        ...
        assert!(matches!(act, Action::Filter(FilterAction::SuggestionDown)));
    }
    ...
    fn up_without_suggestions_is_input() {
        let state = filter_state(false, &[]);
        let act = handle(&state, key(KeyCode::Up));
        assert!(matches!(act, Action::Filter(FilterAction::Input(_))));
    }
    ...
    fn tab_applies_suggestion() {
        ...
        assert!(matches!(act, Action::Filter(FilterAction::ApplySuggestion)));
    }
    ...
    fn enter_no_suggestions_applies_filter() {
        ...
        assert!(matches!(act, Action::Filter(FilterAction::Apply)));
    }
