use crate::{query, query_builder, Core, CoreError, CoreEvent, ErrorKind, SortDirection};
...
mod tests {
    ...
    async fn test_apply_order_no_effective_sql() {
        ...
        let events = core
            ...
            .await;
        assert!(matches!(&events[0], CoreEvent::Error(e) if e.message.contains("No active query")));
    }
